...
while True:
    ...
    # ===================== KFS DETECTION =====================
    results = kfs_model(frame, verbose=False)
        ...
        if roi.size != 0:
            ...
            if sym_results and len(sym_results[0].boxes) > 0:
                ...
                if conf_sym > SYMBOL_CONF:
                    ...
                    # draw symbol box on original frame
                    cv2.rectangle(frame,
                                  (x1 + sx1, y1 + sy1),
                                  (x1 + sx2, y1 + sy2),
                                  (0,255,0), 2)
            ...
            # ================= TF DISPLAY =================
            disp = crop if crop is not None else roi.copy()

            color = (0,255,0) if last_label == "Real" else (0,0,255)

            cv2.putText(disp, f"{last_label} ({last_score:.2f})",
